...
- **External binaries are resolved, never assumed**: `resolveTmuxBin`, `resolveGlabBin`, and `resolveGitBin` (`internal/tn/serve.go`, `internal/tn/reaper.go`) try the usual Homebrew and system locations before falling back to `$PATH`, because launchd's environment has no Homebrew on `PATH`. The operating contract `ORCHESTRATOR.md` is embedded in the binary (root `embed.go`, package `tasknotescli`), synced to `~/.config/tn/ORCHESTRATOR.md` at startup by `ensureManagedOrchestratorDoc` with conffile semantics (an untouched copy is refreshed, an edited copy is kept and the newer contract written to `ORCHESTRATOR.md.new` beside it; a header line carrying the embed's sha256 tells the two apart), then located by `resolveOrchestratorDoc` (env `TN_ORCHESTRATOR_DOC`, `serve.json` `orchestratorDoc`, `~/.config/tn/`, next to the binary, cwd) and logged at startup.
