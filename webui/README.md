...
directly into the Go binary via `go:embed` (see `embed.go` in this directory). The daemon never runs
Node or a JS toolchain itself.

```bash
pnpm install
pnpm dev      # Vite dev server with HMR, talks to a running tn serve instance
pnpm build    # emits dist/, which embed.go embeds via `//go:embed all:dist`
pnpm lint
```
