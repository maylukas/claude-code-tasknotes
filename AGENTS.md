...
Go CLI + bridge daemon connecting the Obsidian TaskNotes plugin to Codex agent
...
| `internal/tn/reaper.go` | Worktree reaper: removes finished `.Codex/worktrees/agent-*` worktrees when clean + pushed + unlocked + old enough (never `--force`, branch `-d` only when merged), `/status.worktrees`, `POST /worktrees/reap`, `tn worktrees` — see `docs/design/SPEC-serve.md` "Worktree reaper". Tests build real temp git repos |
| `internal/tn/creds.go` | Codex.ai credential profiles: Keychain abstraction (`keychainStore`, real impl shells out to `/usr/bin/security`), profile save/list/use/rm, usage-limit auto-swap + parked-pane nudge, `GET /creds` / `POST /creds/swap`, `tn creds` CLI — see `docs/design/SPEC-serve.md` "Credential profiles & auto-swap". Tests inject a fake keychain; never call `security` |
...
| `skills/tasknotes/` | Codex skill, exposed via this repo being a plugin |
| `.Codex-plugin/` | Plugin + local-marketplace manifests (marketplace `tasknotes-local`, user-scope install) |
| `hooks/Codex-hook.sh` | Codex `PermissionRequest`/`PermissionDenied` hook script — one script for both events, user-installed into their own `~/.Codex/settings.json` (never written by the daemon), POSTs to `POST /hooks/permission-request` or `/hooks/permission-denied` — see `docs/design/SPEC-serve.md`'s "PermissionRequest hook detection" section |
...
- Credential profiles: secrets live in the Keychain (Codex's own item is service
  `Codex-credentials`; tn's saved profiles are service `tn-credential-profiles`,
  ...
  a swap switches EVERY Codex session on this machine.
...
- Dashboard note: `~/Documents/MyVault/Codex Sessions.md` (generated — never edit).
...
3. **Repo note (for the Obsidian picker)**: create
   `~/Documents/MyVault/Repos/<slug>.md` — the `Codex-project` user field's
...
4. **Route a task to it**: set the task's `Codex-project` field to the slug (or repo
   note link), tag it `Codex`/`spawn` or write an `@Codex:` line → daemon spawns the
...
- **The moment you tell a user to reference a path, that path is a published interface,
  ...
  absolute path to that file into their `~/.Codex/settings.json`. The user did, via
  team-lead. This agent then renamed the file to `hooks/Codex-hook.sh` (a genuinely
  ...
  `~/.Codex/settings.json` for a hook script, `~/.config/tn/` for daemon config, etc.
  ...
  path for this specific script is `hooks/Codex-hook.sh`; if it moves again, update
  the user's `~/.Codex/settings.json` (or confirm with whoever coordinates that it's
