...
For work with an associated merge/pull request (`customProperties.mr` — a GitLab MR or a GitHub PR, detected from the URL or forced per project via `codeHost`), a background MR watcher polls the appropriate provider CLI (`glab` or `gh`) every 5 minutes over every non-archived, non-completed task regardless of status — only tasks that actually have `customProperties.mr` set ever reach the provider, so the call rate stays bounded to MR/PR-bearing work — reacting only to an **observed change** in the MR/PR's state since the last check:
...
While an MR/PR sits `"opened"`, the same pass also checks its review discussions (GitLab discussions or GitHub review threads). A new comment from anyone other than the MR/PR's own author (never the acting agent's own reply) gets a bridge-attributed note on the task summarizing who commented, how many discussion threads are still unresolved, and an excerpt of the earliest new comment, plus a message to the task's owner (or the project's fallback agent/queue, the same routing `transitionMRClosed` uses) telling it to address the feedback, push, then reply/resolve. A thread resolving with no accompanying new comment just updates the tracked open-thread count silently. `/status.needsActionTasks[].mrOpenThreads` (and the matching field on an agent's own `ownedTasks`) surfaces that count, and the dashboard's Needs-your-action MR line appends `, N unresolved thread(s)` when it's nonzero.
