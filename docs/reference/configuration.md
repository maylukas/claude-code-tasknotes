...
| `TN_NO_MRINCREMENTAL` | `"1"` disables the MR watcher's incremental (GitLab-only) polling, reverting every task to one `glab`/`gh` call per pass regardless of whether its MR actually changed. |
...
> **Note:** `ORCHESTRATOR.md` (the operating contract read by daemon-spawned sessions) is located at startup, in this order: the `TN_ORCHESTRATOR_DOC` environment variable, `serve.json`'s `orchestratorDoc` key, `~/.config/tn/ORCHESTRATOR.md`, `ORCHESTRATOR.md` next to the `tn` binary, then `./ORCHESTRATOR.md` in the daemon's working directory. The first existing file wins; the daemon logs `serve: orchestrator contract: <path>` at startup, or a warning naming the two ways to set it when none is found. The `~/.config/tn/` copy is managed with conffile semantics: the binary embeds `ORCHESTRATOR.md` and writes it there at startup when absent, so a plain install always has a contract. The file starts with a one-line HTML comment recording the sha256 of the contract it was written from. While the body still matches that hash the daemon refreshes the file whenever a newer binary carries a different contract. Once you edit it, the daemon never overwrites it; if a later binary's contract differs from the one your edits were based on, it writes that contract to `~/.config/tn/ORCHESTRATOR.md.new` beside yours and logs it, and it is up to you to merge (copying `.new` over the file makes it untouched again, and the stale `.new` is removed on the next start). A file with no header is treated as edited unless it is byte-identical to the embedded contract. `orchestratorDoc` and `TN_ORCHESTRATOR_DOC` are for keeping the contract at a different path altogether; they do not affect the managed copy.
...
> **Note:** for GitLab, `TN_NO_MRINCREMENTAL` (above) controls whether the watcher polls per-project or per-task. By default (unset), MR-bearing tasks routed to the same GitLab project are grouped and checked with one `glab api projects/:id/merge_requests?...&updated_after=...` listing call per project per pass instead of a `glab mr view` per task, so `glab` call volume scales with the number of *projects* with open MRs, not the number of tasks — state (`glab mr view`) and discussions (`glab api .../discussions`) calls still happen, but only for a task the listing reports changed, a task never observed before, or (every ~30 minutes) every currently-open task in the group as a belt-and-braces refresh. `TN_NO_MRINCREMENTAL=1` reverts to a `glab mr view` (plus, while open, a discussions fetch) for every MR-bearing task every pass, same as GitHub always does today. A fresh state file (no `mrCursors` entry yet for a project) costs one full listing of that project's merge requests on its first pass, capped at 10 pages of 100.
